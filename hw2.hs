...
-- 	 (edited to specify what empty string returns, some clarification added).checkStrings [["Adf", "asdf", "asd", ""],["asd"], ["asd", "as"]]
...
toZero :: [[String]] -> [[String]] 
toZero = foldr (\x acc ->  check x : acc ) []

check :: [String] -> [String]
check xs = map(\x -> if x == "" then "0" else x) xs
...
foldLambda :: [Int] -> [Int]
foldLambda a = foldl (\acc x -> acc ++ [fst x * snd x]) [] $ joinIt a
